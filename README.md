...
| 현재 버전 | `v4.2.19` (`js/version.js` 기준, 2026-08-25) |
| 작성 기준 | `develop` 브랜치 현재 코드와 `APP_VERSION.history` |

## 현재 버전 요약

- **v4.2.19 주요 변경 사항**:
  - 구매 관리 전체 탭에서 현재 상태를 처리한 담당자와 시각을 표시합니다. 구매 신청·취소 상태는 저장된 처리자 정보가 없어 표시하지 않습니다.

- **v4.2.18 주요 변경 사항**:
  - 구매 주문 프로필 조회 RPC의 `department_id` 컬럼 충돌을 수정해, 관리 부서 밖 주문의 신청자·부서 정보를 정상 표시합니다.
  - 운영 DB에는 수정된 `docs/TASK-112_purchase_order_profile_context_visibility.sql`을 다시 적용해야 합니다.

- **v4.2.17 주요 변경 사항**:
  - 구매 관리의 기존 주문 목록 권한은 그대로 유지하면서, 표시된 주문의 신청자·부서·이전 처리자 정보를 관리 부서와 관계없이 확인할 수 있도록 보완했습니다.
