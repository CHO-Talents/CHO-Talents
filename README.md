...
- **v3.20.0 주요 변경 사항**:
  ...
  - QR 관리: 복사 버튼 제거, 검색(설명/코드) + 필터(대상/조건/생성일) 추가
  - QR 수령: 계정 유형(학생/교사)과 QR 대상 불일치 시 수령 차단
  - QR 수령: 위치 범위 초과 시 에러에 주소 명칭 표시
  - 사용자 관리: 페이징 처리 (모바일 10개/PC 25개)
  - DB: `talent_qr_codes`에 `repeat_type`, `repeat_days`, `repeat_weeks` 컬럼 추가
- **v3.19.0 주요 변경 사항**:
