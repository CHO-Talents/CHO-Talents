...
| 현재 버전 | `v3.38.1` (`js/version.js` 기준, 2026-06-12) |
| 작성 기준 | `develop` 브랜치 현재 코드와 `APP_VERSION.history` |

## 현재 버전 요약

- `APP_VERSION.current`는 `3.38.1`으로 갱신되어 있습니다.
- **v3.38.1 주요 변경 사항**:
  - 네비게이션 배지: 관리 메뉴는 처리 가능한 가입 신청 + 부서 이동 건수를 표시
  - 상품 배지: 권한 범위에서 처리 가능한 구매 건수를 표시하며, 구매 담당 교사는 전체 부서 주문을 집계
  - 운영 배지: 부장 교사(80+) 이상에서 미확인 ERROR 이상 로그 건수를 호출
  - 구매 담당 교사(`purchase_teacher`)가 전체 부서 구매 처리와 배지 집계에 포함
- **v3.38.0 주요 변경 사항**:
