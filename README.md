...
| `admin/users.html` | 60 | 사용자 목록, 교사/학생 로우 클릭 상세 모달, 권한 범위 내 수정/삭제/비밀번호 초기화, 가입 신청 처리, 부서 이동 요청/승인. 전도사님·관리자는 모든 사용자 아이디를, 부장·구매 담당·부서 담당 교사는 담당 부서의 일반 교사·학생 아이디를 확인 |
...
- 아이디(`username`)는 전도사님·관리자에게 전체 표시되고, 부장·구매 담당·부서 담당 교사에게는 담당 부서의 일반 교사·학생만 표시됩니다. 동명이인은 표시명에 번호를 붙여 구분합니다.
