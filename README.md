...
| 현재 버전 | `v4.2.14` (`js/version.js` 기준, 2026-08-24) |
| 작성 기준 | `develop` 브랜치 현재 코드와 `APP_VERSION.history` |

## 현재 버전 요약

- **v4.2.14 주요 변경 사항**:
  - 구매 담당 교사는 구매 관리에서 모든 부서·사용자 유형·권한의 주문자 이름과 부서를 확인할 수 있습니다.
  - 구매 담당 교사는 전체 부서 주문의 구매 신청·상품 준비·구매 확정·지급 완료·취소·준비 단계 되돌리기를 관리할 수 있습니다. 운영 DB에는 `docs/TASK-109_purchase_teacher_global_purchase_management.sql` 적용이 필요합니다.
...
- `APP_VERSION.current`는 `4.2.14`로 갱신되어 있습니다.
- **v3.88.0 주요 변경 사항**:
