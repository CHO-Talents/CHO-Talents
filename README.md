...
| 현재 버전 | `v3.19.1` (`js/version.js` 기준, 2026-06-01) |
| 작성 기준 | `develop` 브랜치 현재 코드와 `APP_VERSION.history` |

## 현재 버전 요약

- 모든 HTML의 JS 캐시 버스팅과 `APP_VERSION.current`는 `3.19.1`으로 맞춰져 있습니다.
- **v3.19.1 주요 변경 사항**:
  - QR 수령: 관리자(100+) 외 QR 코드 텍스트 값 마스킹 처리
  - 모바일 반응형: admin/common CSS 테이블 `min-width` 강제값 제거, 화면 크기에 맞게 자동 조절
  - 모바일: 480px 이하 요약카드/테이블 폰트 크기 최적화
- **v3.19.0 주요 변경 사항**:
...
- **v3.15.0 주요 변경 사항**:
  ...
  - 사용자 가이드: `body.page-body` 클래스 추가로 타이틀 짤림 해결
