...
| 부장 교사 | `chief` | 80 | `index.html` | 대시보드, 사용자 관리, 부서/보고서/서비스 통계, 달란트 반환 처리 (모든 부서의 일반 교사·학생 목록 조회, 담당 부서만 관리) |
| 구매 담당 교사 | `purchase_teacher` | 70 | `index.html` | 담당 부서 사용자 관리, 구매 관리에서 전체 부서 주문 처리 |
...
| `admin/users.html` | 60 | 사용자 목록, 교사/학생 로우 클릭 상세 모달, 권한 범위 내 수정/삭제/비밀번호 초기화, 가입 신청 처리, 부서 이동 요청/승인. 전도사님·관리자는 모든 사용자 아이디를, 부장 교사는 모든 부서의 일반 교사·학생 아이디를 확인하되 담당 부서 밖 계정은 목록으로만 조회 |
...
- 아이디(`username`)는 전도사님·관리자에게 전체 표시됩니다. 부장 교사는 모든 부서의 일반 교사·학생 아이디를 목록에서 보되, 담당 부서 밖 계정은 상세·관리할 수 없습니다. 구매 담당·부서 담당 교사는 담당 부서의 일반 교사·학생 아이디만 볼 수 있습니다. 동명이인은 표시명에 번호를 붙여 구분합니다.
