...
- **상품 구매 시스템** (v3.9.0 신규):
  ...
  - 구매 신청 시 달란트는 즉시 차감되지 않고 `pending_talent`(사용 대기)로 관리
  - "상품 구매" 상태 변경 시 실제 달란트가 차감
- **달란트 관리** 개편: 항목 즉시 지급 → 체크박스 선택 + 일괄 지급 확정 방식
  - 이미 지급된 항목은 자동으로 표시
  - 부장 교사(80+) 이상은 지급된 달란트를 사유 입력 후 반환 처리 가능
  - 상세 모달에 지급자 정보 표시
- **에러 처리 강화**:
  - `tErr()` 공통 에러 번역 함수: 영문 DB/RPC 에러를 한글로 자동 변환 (25개 패턴)
  - 전체 페이지 에러 로깅: 모든 기능의 성공/실패/거부를 `activity_logs`에 기록
  - DB 컬럼 미존재 시 한글 안내 메시지 제공
- **명칭 통일**: "물품"→"상품", "부장"→"부장 교사", "부서관리자"→"부서 담당 교사"
- **로그 관리**: 소프트 삭제(`is_deleted=true`) 방식, 실제 DELETE는 관리자가 SQL Editor에서 직접 수행
  - 삭제 관련 버튼은 관리자(100+)만 표시
