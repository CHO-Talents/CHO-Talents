...
if [ "${#EXTRA_SQL_PATHS[@]}" -eq 0 ]; then
  ...
  DEFAULT_PRODUCT_SUGGESTION_ADOPTION_REWARD_ITEM_CONFIG_SQL="$ROOT_DIR/docs/TASK-113_product_suggestion_adoption_reward_item_config.sql"
  ...
  if [ -f "$DEFAULT_PRODUCT_SUGGESTION_ADOPTION_REWARD_ITEM_CONFIG_SQL" ]; then
    EXTRA_SQL_PATHS+=("$DEFAULT_PRODUCT_SUGGESTION_ADOPTION_REWARD_ITEM_CONFIG_SQL")
  fi
