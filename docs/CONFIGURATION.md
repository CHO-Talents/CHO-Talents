...
> v3.87.0 기능을 사용하려면 `docs/TASK-089_product_suggestion_slack_notifications.sql`을 `TASK-088` 다음에 적용하고, 최신 `supabase/functions/slack-notify` Edge Function을 배포합니다. 기존 `SLACK_WEBHOOK_OPERATIONS` Secret을 사용해 상품 추천 등록 완료와 투표 종료 결과를 운영관리 채널로 전송하며, 새 Secret은 필요하지 않습니다.
