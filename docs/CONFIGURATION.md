...
> v3.86.0 기능을 사용하려면 `TASK-085_product_suggestions.sql`, `TASK-086_product_suggestion_admin_decision.sql`, `TASK-087_product_suggestion_detail_image.sql` 다음에 `docs/TASK-088_product_suggestion_adoption_talent.sql`을 적용합니다. 이 마이그레이션은 채택 추천 등록자에게 `상품 추천 채택` 1달란트를 건별 자동 지급하고, 중복 방지 보상 원장·채택 상품 순번 999·지급 로그/출처 코드를 추가합니다.
