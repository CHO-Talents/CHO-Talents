...
-- 2. 페이지 기능 관리 테이블
CREATE TABLE IF NOT EXISTS user_page_features (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  page_id TEXT NOT NULL,
...
ALTER TABLE user_page_features ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "user_page_features_select" ON user_page_features;
CREATE POLICY "user_page_features_select" ON user_page_features
  FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "user_page_features_insert" ON user_page_features;
CREATE POLICY "user_page_features_insert" ON user_page_features
  FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND get_permission_rank(permission_level) >= 90)
  );

DROP POLICY IF EXISTS "user_page_features_update" ON user_page_features;
CREATE POLICY "user_page_features_update" ON user_page_features
  FOR UPDATE USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND get_permission_rank(permission_level) >= 90)
  );

DROP POLICY IF EXISTS "user_page_features_delete" ON user_page_features;
CREATE POLICY "user_page_features_delete" ON user_page_features
  FOR DELETE USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND get_permission_rank(permission_level) >= 90)
  );
