...
const APP_VERSION = {
  ...
  history: [
    ...
    },
    {
      version: '4.2.13',
      date: '2026-08-20',
      title: '부장 교사 사용자 목록 부서 필터 복구',
      changes: [
        '전체 부서의 교사·학생 목록을 조회하는 부장 교사(80+)에게 사용자 관리 부서 필터를 표시하도록 수정했습니다.'
      ]
    },
    {
      version: '4.2.12',
      date: '2026-08-20',
      title: '부장 교사 교사 유형 목록 누락 수정',
      changes: [
        '사용자 목록의 교사·학생 판별을 권한 코드가 아닌 사용자 유형으로 수정해 부장 교사 본인과 부서 담당 교사 등 교사 유형 계정도 목록에 표시합니다.',
        '부장은 모든 부서 교사·학생을 목록에서만 확인하고, 담당 부서 계정에만 상세·관리 기능을 사용할 수 있습니다. 운영 DB에는 TASK-108 SQL 적용이 필요합니다.'
      ]
    },
    {
      version: '4.2.11',
      date: '2026-08-20',
      title: '부장 교사 사용자 목록 조회 범위 복원',
      changes: [
        '부장 교사는 모든 부서의 교사·학생 계정을 목록과 아이디로 확인할 수 있으며, 담당 부서 밖 계정은 상세·수정·이동·비밀번호 초기화·삭제 없이 목록만 조회합니다.',
        '부장 교사의 목록 조회 범위를 화면과 사용자 조회 RPC·profiles 정책에 반영했습니다. 운영 DB에는 TASK-108 SQL 적용이 필요합니다.'
      ]
    },
    {
      version: '4.2.10',
      date: '2026-08-20',
      title: '사용자 관리 담당 부서 계정 조회 통일',
      changes: [
        '관리자·전도사님은 모든 사용자 계정의 아이디를 확인하고, 구매 담당·부서 담당 교사는 담당 부서의 일반 교사·학생 계정만 확인할 수 있습니다.',
        '구매 담당·부서 담당 교사도 사용자 관리 화면에 접근할 수 있도록 하며, 화면·사용자 조회 RPC·profiles 조회 정책에 동일한 담당 부서 범위를 적용했습니다. 운영 DB에는 TASK-107 SQL 적용이 필요합니다.'
      ]
    },
    {
      version: '4.2.9',
      date: '2026-08-19',
      title: '즐겨찾기 전체 메뉴 동기화와 운영 문서 최신화',
      changes: [
        '메인 즐겨찾기 설정에 공지 사항, 월별 달란트 관리, 구매 통계, Slack 알림 룰, 서비스 통계를 추가해 네비게이션에서 선택 가능한 메뉴를 모두 즐겨찾기로 등록할 수 있습니다.',
        'README, 사이트 안내서, 아키텍처 문서와 역할별·운영 룰 가이드를 v4.2.9 현재 메뉴·권한·달란트 지급 대상일 기준으로 동기화했습니다.'
      ]
    },
    {
      version: '4.2.8',
      date: '2026-08-19',
      title: '달란트 지급 대상일 일요일 통일',
      changes: [
        '모든 달란트 지급은 발생일이 일요일이면 당일, 월~토요일이면 다음 일요일을 grant_date로 기록합니다.',
        '월별 달란트 관리에서 선택한 일요일 지급일은 그대로 유지하며, 일반 지급·수동 지급·QR 수령과 DB 저장 경로에도 같은 기준을 적용했습니다. 운영 DB에는 TASK-105 SQL 적용이 필요합니다.'
      ]
    },
    {
      version: '4.2.7',
      date: '2026-08-19',
      title: 'QR 달란트 발생 시각·지급 대상일 분리',
      changes: [
        'QR 수령 거래에 한국 시간 기준 지급 대상일(grant_date)을 명시 저장하고, 수령 발생 시각(created_at/scanned_at)은 별도로 유지합니다.',
        'QR 관리와 수령 내역에서 지급 대상일과 실제 수령 발생 시각을 각각 확인할 수 있습니다. 운영 DB에는 TASK-104 SQL 적용이 필요합니다.'
      ]
    },
    {
      version: '4.2.6',
      date: '2026-08-19',
      title: '일반 달란트 지급일 기준 완료 판정',
      changes: [
        '일반 달란트 관리의 출석 및 항목별 지급 완료 여부를 거래 생성시각이 아닌 grant_date(실제 지급 대상일) 기준으로 판단하도록 변경했습니다.'
      ]
    },
    {
      version: '4.2.5',
      date: '2026-08-19',
      title: '달란트 대상자 소속 우선 정렬',
      changes: [
        '일반·월별 달란트 관리 대상자 목록에서 로그인 사용자의 소속 부서·반을 먼저, 같은 부서의 나머지 인원을 다음으로 표시합니다.',
        '그 밖의 대상자는 부서·반·이름 오름차순을 유지합니다.'
      ]
    },
    {
      version: '4.2.4',
      date: '2026-08-19',
      title: '달란트 대상자 조회·지급 권한 범위 정렬',
      changes: [
        '최고관리자를 제외하고, 관리자·전도사님은 전체 부서의 모든 계정을 조회·지급하며 부장 교사는 전체 조회하되 관리 부서에만 지급합니다.',
        '구매 담당·부서 담당 교사는 관리 부서 전체 계정을 조회·지급하며, 일반 교사는 자기 부서·자기 반 학생만 조회·지급합니다.',
        '월별/일반 달란트 관리 화면과 profiles·달란트 이력 RLS 및 지급 RPC에 같은 규칙을 적용했습니다. 운영 DB에는 TASK-103 SQL 적용이 필요합니다.'
      ]
    },
    {
      version: '4.2.3',
      date: '2026-08-18',
      title: '월별 달란트 관리 조회·모바일 그리드 개선',
      changes: [
        '연도와 월을 각각 선택해 원하는 지급 월로 바로 이동할 수 있도록 하고, 이전·다음 달 및 이번 달 이동 버튼은 유지했습니다.',
        '월별 지급 대상은 부서, 반, 이름 오름차순으로 정렬하고 이름·소속 열 너비를 축소했습니다.',
        '모바일 그리드에서는 잔액과 지급 항목 목록을 숨기며, 날짜 셀을 누르면 해당 날짜의 지급 항목을 팝업으로 확인할 수 있습니다.',
        '연도 선택 범위를 현재 연도와 과거 5년으로 제한하고, 월 이동 화살표도 이 범위를 벗어나지 않도록 했습니다.',
        '선택한 지급 항목이 모두 지급된 날짜 셀은 체크박스 대신 “✓ 지급 완료” 상태로 명확히 표시합니다.',
        '모바일 표를 5주가 한 화면에 보이도록 고정 레이아웃과 더 작은 열·행 간격으로 압축했습니다.',
        '대상자별 날짜 셀을 지급 내역 유무에 따라 색상과 “지급/미지급” 상태로 구분하고, 지급 항목 수와 지급 가능 항목 수를 함께 표시합니다.',
        '달란트 관리와 월별 달란트 관리 목록에서 로그인 사용자의 부서·반 대상자를 최우선으로 표시하고, 이후에는 부서·반·이름 오름차순을 유지합니다.'
      ]
    },
    {
      version: '4.2.2',
      date: '2026-08-18',
      title: '사용자 계정 조회 권한 범위 조정',
      changes: [
        '사용자 관리 화면을 부장 교사(80) 이상에게 열고, 부장 교사는 담당 부서의 일반 교사와 학생 계정만 조회하도록 제한했습니다.',
        '전도사님(90) 이상은 관리자와 동일하게 전체 계정을 조회할 수 있도록 조정했으며, 관리자 관리 화면은 전도사님 이상으로 제한했습니다.',
        '화면 필터 우회를 막기 위해 profiles 조회 정책과 admin_list_users RPC에 같은 권한 범위를 적용하는 TASK-102 SQL을 추가했습니다.'
      ]
    },
    {
      version: '4.2.1',
      date: '2026-08-12',
      title: '추천 상품 투표 진행률 과반 기준 보정',
      changes: [
        '투표 진행률을 총 투표 수가 아닌 찬성·반대 중 더 많은 한쪽의 득표 수로 계산하도록 수정했습니다.',
        '한쪽 득표가 등록 시점 투표 정원의 과반을 초과하는 데 필요한 표 수에 도달할 때만 100%로 표시됩니다.'
      ]
    },
    {
      version: '4.2.0',
